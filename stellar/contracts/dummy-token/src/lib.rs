...
impl DummyToken {
    pub fn __constructor(e: &Env, owner: Address, initial_supply: i128) {
        Base::set_metadata(e, 18, String::from_str(e, "DummyToken"), String::from_str(e, "DTK"));
...
impl Pausable for DummyToken {
    ...
    fn pause(e: &Env, caller: Address) {
        ...
        let owner: Address = e.storage().instance().get(&OWNER).expect("owner should be set");
        if owner != caller {
            panic_with_error!(e, DummyTokenError::Unauthorized);
        }
...
#[contractimpl]
impl TokenInterface for DummyToken {
    ...
    fn transfer_from(e: Env, spender: Address, from: Address, to: Address, amount: i128) {
        Base::set_allowance(&e, &from, &spender, amount, e.ledger().max_live_until_ledger());
